...
- [x] TASK-0078（NoteLesson 靜默丟欄位）dev 交付完，狀態 in_review、我掛 dev。actor 退回 --persona／title+tags 進 jsonl（沒給不寫鍵）／不認得的參數在 append 前擋下。⚠ 第三格只修 NoteLesson 一支 —— ArgsSpec 表達不出完整字彙表（它只有 Required/Aliases），全體 Cmd 的通解要先給 spec 層加欄位並連 python 預檢一起改，那是另一張單，我沒擴射程。另：驗收在共享 lessons.jsonl 留了兩筆 test 列，沒刪。  <!-- 2026-09-04T07:34:34.343975Z -->
